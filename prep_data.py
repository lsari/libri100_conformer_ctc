...
def extract_logmel(audio_path):
    ...
    win_length = n_fft
    hop_length = int(sample_rate * WINDOW_STRIDE)
    mel_spec = librosa.feature.melspectrogram(y, sample_rate, n_fft=512, hop_length=hop_length, n_mels=80)
    log_mel_spec = librosa.power_to_db(mel_spec)

    # print(log_mel_spec.shape)
    # mean = np.mean(log_mel_spec, 1, keepdims=True)
    # std = np.std(log_mel_spec, 1, keepdims=True)
    # log_mel_spec -= mean
    # log_mel_spec /= std
    return log_mel_spec.T, duration
...
if __name__ == "__main__":
    ...
    save_dir = os.getcwd() + "/data/logmel/"
    ...
    for subset in subset_list:
        ...
        for i, row in df.iterrows():
            S, duration = extract_logmel(row["file"])
